...
class CartAPI {
    public static function read(String $account_id) {
        ...
        $res2 = Mysqllib::mysql_get_data_from_query($conn, "SELECT cart_item.*, product.title, product.imgPath FROM `cart_item` INNER JOIN `product` ON cart_item.productId=product.id AND cart_item.cartId={$res->message[0]["id"]}");
        if(!$res2->status) {
            return new ResponseModel(false, "Error");
        }
