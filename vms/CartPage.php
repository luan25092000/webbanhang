				...
				<table v-if="cartItems.length" class="table product-list">
					...
					<tbody>
							<tr v-for="item in cartItems" :key="item.id">
								<td class="name"><a :href="'/product-detail/'+item.productId">{{item.title}}</a></td>
								...
								<td class="quantity">
									<div class="quantity-control" data-quantity="">
										...
										<input type="number" class="quantity-input" v-model="item.quantity" @change="adjustQuantity(item)"
...
var vmTable = new Vue({
	...
	methods: {
		adjustQuantity(item, quantity = 0) {
			// Set quantity
			if(quantity) {
